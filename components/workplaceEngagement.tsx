...
interface workFirstDataDataItem {
  title: string;
  img: string;
  description: { heading: string; subitem: string }[];
}

interface workSecondDataDataItem {
  title: string;
  points: string[];
}
interface PatientCareDesignProps {
  workFristData: workFirstDataDataItem[];
  workSecondData: workSecondDataDataItem[];
}
      ...
      <div className="flex flex-wrap justify-center gap-6">
        {workSecondData.map((item, index) => (
          <Card
            key={index}
            className={cn(
              "flex-1 transform p-4 shadow-[-1px_-1px_37px_0px_rgba(0,_0,_0,_0.1)] transition-transform duration-300 ease-in-out hover:-translate-y-2",
